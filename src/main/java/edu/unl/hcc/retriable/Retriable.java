...
public abstract class Retriable<T> {
    ...
    public static long defaultTimeOut = 60 * 1000;

    public static long defaultSleepInterval = 5 * 1000;
    ...
    protected long maxSleepInterval;
    ...
    public Retriable() {
        ...
        maxSleepInterval = defaultSleepInterval;
     ...
     * Constructor.
     *
     * @param wt Count of max waiting time
     */
    public Retriable(long wt) {
        this();
        if(wt<defaultSleepInterval) {
            maxSleepInterval=wt;
        }
        maxWaitingTime = wt;
    }
    ...
    /**
     * Gets sleep interval of connecting attempts.
     *
     * @return max sleep interval
     */
     ...
     *           flag
     */
    public T go() throws UtilException, InterruptedException {
        int attempts = 0;
        boolean success = false;
        long lastTime = 0;
        while (!success && attempts < getMaxAttempts() && lastTime < getMaxTimeout()) {
            attempts++;
            ...
            Thread.sleep(getMaxSleepInterval());
            lastTime += getMaxSleepInterval();
        }
        if (!success) {
            LOGGER.error("{}: unsuccessful after {} attempts or wait longer than {}.", getName(), attempts, lastTime);
            giveUp();
        }
